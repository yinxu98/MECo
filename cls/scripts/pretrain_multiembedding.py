...
def train(cfg):
    ...
    for epoch in range(cfg.runner.epoch):
        ...
        for img in data_loader:
            ...
            loss_sim_encoder = torch.FloatTensor([0]).cuda()
            for k in range(n_p):
                for i in range(n_r):
                    for j in range(n_r):
                        if (i != j):
                            loss_sim_encoder += criterion_sim(
                                ls_p[i][k], ls_r[j]).mean()
            loss_sim_encoder = -1 / n_p / n_r / (n_r - 1) * loss_sim_encoder

            loss_sim_predictor = torch.FloatTensor([0]).cuda()
            for k in range(n_r):
                for i in range(n_p):
                    for j in range(i + 1, n_p):
                        loss_sim_predictor += criterion_sim(
                            ls_p[k][i], ls_p[k][j]).mean()
            loss_sim_predictor = -2 / n_r / n_p / (n_p -
                                                   1) * loss_sim_predictor

            loss_sim = loss_sim_encoder + loss_sim_predictor
        ...
        if (epoch + 1) % cfg.runner.save_interval == 0:
            save_checkpoint(
                model.ls_backbone[0].state_dict(),
